...
//        GLES30.glDisableVertexAttribArray(aTextureCoordLocation);
    ...
    public void run() {
         ...
         */
        initTexture();
        //与test1 test2不同之处 双PBO
        int[] pboId = new int[2];
        GLES30.glGenBuffers(2, pboId,0);
        ...
        GLES30.glBindBuffer(GLES30.GL_PIXEL_PACK_BUFFER, pboId[1]);
        GLES30.glBufferData(GLES30.GL_PIXEL_PACK_BUFFER, 4*ySize, null, GLES30.GL_DYNAMIC_READ);
        GLES30.glBindBuffer(GLES30.GL_PIXEL_PACK_BUFFER, 0);
        while (!quit) {
            /**
...
//            GLES30.glReadBuffer(GLES30.GL_BACK);
            ...
            Log.e("aa", "tttreadPixel=" + (System.currentTimeMillis() - a));//数据转换时间太长
