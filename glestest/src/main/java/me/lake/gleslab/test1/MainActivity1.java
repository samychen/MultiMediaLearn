...
public class MainActivity1 extends AppCompatActivity implements TextureView.SurfaceTextureListener {
    int w = 1280;
    int h = 720;
    int ySize = w * h;
    int uvSize = ySize >> 1;
    ...
    GLRenderThread1 glRenderThread;
...
//        width = dm.widthPixels;
    ...
    public void onSurfaceTextureAvailable(SurfaceTexture surface, int width, int height) {
        startCamera();
        Log.e("tag", "onSurfaceTextureAvailable() called with: surface = [" + surface + "], width = [" + width + "], height = [" + height + "]");
        glRenderThread = new GLRenderThread1(this, surface);
...
//        cam.addCallbackBuffer(new byte[(w * h * 3) / 2]);
        cam.setPreviewCallbackWithBuffer(new Camera.PreviewCallback() {
            ...
            public void onPreviewFrame(byte[] data, Camera camera) {
                ...
                glRenderThread.queueYUV(y, u, v);//OpenGL绘制特效视频
                HomeActivity.ndkdraw(sv_image.getHolder().getSurface(), data, w, h, data.length);//ndk绘制原始视频
